...
def delete_section_note(purpose_,list_widgets):
    ...
    if len(value_combobox) > 1:
        # deletewindow formation
        deletewindow = Tk()
        deletewindow.geometry('300x300')
        deletewindow.title('AIY Notes')
        deletewindow.resizable(False, False)
        deletewindow["bg"] = '#bfbfbf'

        # widgets
        label_name = Label(deletewindow, text=name_label, font="calbri 16")
        label_instruction = Label(deletewindow, text=instructions)
        Next_ = Button(deletewindow, text="Next", command=lambda: delete(purpose_, list_widgets))
        Back_ = Button(deletewindow, text="Back", command=lambda: deletewindow.destroy())
        combobox_ = ttk.Combobox(deletewindow, text="Options")
        combobox_["values"] = value_combobox

        # widget placement
        label_name.place(relx=0.3, rely=0.05)
        label_instruction.place(relx=0.1, rely=0.3)
        combobox_.place(relx=0.1, rely=0.4)
        Next_.place(relx=0.7, rely=0.7)
        Back_.place(relx=0.1, rely=0.7)

        if purpose_ == "note":
            # extra label for section verification
            text_selected_section = "section chose section: " + str(present_section)
            select_section = Label(deletewindow, text=text_selected_section)
            # placement of widget
            select_section.place(relx=0.1, rely=0.2)
        else:
            pass
...
if proceed == True:
    ...
    def maindisplay():
        ...
        save_ = Button(function_panel, text="Save", bg="#000000", command=lambda: donothing())
        quit_ = Button(function_panel, text="Quit", bg="#000000", command=lambda: displaywindow.destroy())
        add_ = Button(function_panel, text=" + Add", bg="#000000", command=lambda: donothing())
        remove_ = Button(function_panel, text="- Remove", bg="#000000", command=lambda: donothing())
        recover_ = Button(function_panel, text="Recover", bg="#000000", command=lambda: donothing())
        about_ = Button(function_panel, text="About", bg="#000000", command=lambda: donothing())
        search_ = Button(function_panel, text="Search", bg="#000000", command=lambda: donothing())
        ...
        section_subtract = Button(section_panel, text="- Section", pady=4, bg="#999999", command=lambda: delete_section_note("section",list_maindisplay_widgets))
        ...
        note_subtract = Button(note_panel, text="- Note",pady=4, bg="#bfbfbf", command=lambda: delete_section_note("note",list_maindisplay_widgets))
