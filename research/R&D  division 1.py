...
if AIY_notes_database == True:
    ...
    while user_accounts is False:
        ...
        if user_accounts is True:
            if "note_A0001_table" in tables:
                pass
            else:
                my_c.execute("use AIY_note_database")
                execute_code_2 = "create table note_A0001_table(Section_no INT,Section_name varchar(150)," \
                                 "Note_no INt,Note_name Varchar(150),Date_creation Date,Delete_note varchar(10)) "
                my_c.execute(execute_code_2)
        else:
            my_c.execute("use AIY_note_database")
            my_c.execute(
                "create table User_accounts(S_Name VarChar(100),DoB DATE,Password VarChar(100),Account_no "
                "VarChar (18),ph_no VarChar(20),email_id VarChar(100))")
            my_c.execute(
                "create table note_A0001_table(Section_no INT,Section_name varchar(150),Note_no INt,Note_name "
                "Varchar(150),Date_creation Date,Delete_note varchar(10)")


def sql_part(purpose_sql, list_info_sql=None):
    if purpose_sql == 'account_creation':
        if list_info_sql is not None:
            sql_creation_code = "input into User_accounts values(%s,%s,%s,%s,%s,%s)"
        ...
        my_c.execute('use AIY_note_database')
        mycursor = my_c.cursor()
        mycursor.execute("SELECT Account_no,Password FROM User_accounts")
        myresult = mycursor.fetchall()
        id_password_verify = None
        if myresult is not None:
            if list_info_sql is not None:
                for info in myresult:
                    if list_info_sql == info:
        ...
        elif id_password_verify == False:
            return 'not found'
        else:
            print(" login sql not working")

    elif purpose_sql == "exit program":
        # anmol's part; refill all present new data into the database
        '''
        could follow the destroy table and create a new one
        or
        could transverse to see what are the changes required for the table and correct only those.
        
        mostly first one is most usefull and saves time when large data are to be transversed
        '''
        print()


def message_window(purpose_mw=None, text_message_mw=None):
    messagewindow = Tk()
    messagewindow.title('AIY Note')
    messagewindow.geometry('500x500')
    messagewindow.resizable(False, False)
    messagewindow["bg"] = '#000000'

    if purpose_mw is not None:
        if purpose_mw == "forget_account":
            if text_message_mw is not None:
                # the label is the text_message_mw
                print()
            else:
                print("Message not available")
        else:
            print("purpose out of range")
    else:
        print("purpose not available ")
...
def activation_procedure(window, list_entry):
    ...
    if window == "login_displaywindow":
        ...
        login_id_entry_allow_ac = "AIY_login"
        login_password_entry_allow_ac = "AIY_password"
        if login_id == login_id_entry_allow_ac and password_ == login_password_entry_allow_ac:
            maindisplay()
        else:
            pass
...
def loginwindow(back_from_window=None, list_window_widget=None):
    ...
    next_l = Button(
        ...
        [
            ...
            activation_procedure('displaywindow', [id_, password_])
        ]
    )
    ...
    loginwindow_list_widgets = (
        wel, signin, loginid, loginpassword, id_, password_, next_l, forgotpassword_l, createaccount_l)
...
def account_creation():  # create accounts display window
    ...
    def verification_password_otp():
        ...
        if password_.get() != "":
            if confirm_password_.get() != "":
                if password != confirm_password_:
                    ...
                    messagebox.showerror("AIY Note", "Confirm passwword is not equal to password, please try again.")
                else:
                    pass
            else:
                pass
        ...
        if otp_create_.get() != "":
            if pincode != "":
                if int(otp_create_.get()) != pincode:
...
def forget_password():
    ...
    def otp_verify():
        if otp_forget_.get() != "":
            if pincode != "":
                if int(otp_forget_.get()) != pincode:
                    otp_forget_.delete(0, END)
                    messagebox.showerror("AIY Note", "OTP is not equal to assigned OTP, please try again.")
                elif int(otp_forget_.get()) == pincode:
                    ...
                    correct_account_forget_user = sql_part('forget_acount',forget_info)
                    if correct_account_forget_user == 'not found':
                        text_messagebox_cafu_f = "The application could not find your account, either try again or" + \
                                          "create an account "
                        messagebox.showerror("AIY Note", text_messagebox_cafu_f)

                    elif correct_account_forget_user is not None:
                        message_window('forget_account', correct_account_forget_user)
...
def _loginwindow_1():
    ...
    next_l1 = Button(
        displaywindow, text="Next", command=lambda:
        [activation_procedure('login_displaywindow', [id_.get(), password_.get()])]
    )
