...
# global references of the program
active_no_sectionpanel = 0
active_no_notepanel = 0
no_sections = 1
no_notes = 1
note_name = []
section_name = []
...
def about_project():
    text = ("This is a computer project undertaken by a few students of The Foundation School, Gunjur,Bangalore.",
    "The aim of the project is to provide a note taking application that not just provide the necessary options and ",
    "features that the people have already been used to but also give ways through which the user's can fully custamise " 
    "their display.")
    messagebox.showinfo("About AIY Note", text)

menubar = Menu(mainwindow)

# file menu
filemenu = Menu(menubar, tearoff=False)
filemenu.add_command(label="New note", command=no_notes)
filemenu.add_command(label="New section", command=new_section)
filemenu.add_command(label="Open note", command=donothing)
filemenu.add_command(label="Save note", command=donothing)
filemenu.add_command(label="Save as...", command=donothing)
filemenu.add_command(label="Close note ", command=notebook.quit)
# To add a separator
filemenu.add_separator()
filemenu.add_command(label="Exit", command=mainwindow.quit)
# To add file tab on the menu bar
...
editmenu.add_separator()
editmenu.add_command(label="Speech", command=donothing)
menubar.add_cascade(label="Edit", menu=editmenu)

# insert menu
insertmenu = Menu(menubar, tearoff=False)
insertmenu.add_command(label="Table", command=donothing)
insertmenu.add_command(label="calender", command=donothing)
insertmenu.add_command(label="Image", command=donothing)
insertmenu.add_command(label="Board", command=donothing)
insertmenu.add_separator()
insertmenu.add_command(label="Checkbox", command=donothing)
insertmenu.add_command(label="Radiobutton", command=donothing)
insertmenu.add_separator()
insertmenu.add_command(label="Encryption", command=donothing)
insertmenu.add_command(label="Lock", command=donothing)
menubar.add_cascade(label="Insert", menu=insertmenu)

# section menu
sectionmenu = Menu(menubar, tearoff=False)
sectionmenu.add_command(label="New", command=new_section)
sectionmenu.add_command(label="Rename", command=donothing)
sectionmenu.add_command(label="Delete", command=donothing)
sectionmenu.add_command(label="Close", command=donothing())
sectionmenu.add_command(label="background", command=donothing)
sectionmenu.add_command(label="Font", command=donothing)
sectionmenu.add_separator()
sectionmenu.add_command(label="Encryption", command=donothing)
sectionmenu.add_command(label="Lock", command=donothing)
menubar.add_cascade(label="Sections", menu=sectionmenu)

# notes menu
notemenu = Menu(menubar, tearoff=False)
notemenu.add_command(label="New", command=new_note)
notemenu.add_command(label="Rename", command=donothing)
notemenu.add_command(label="Delete", command=donothing)
notemenu.add_command(label="Close", command=donothing)
notemenu.add_command(label="background", command=donothing)
notemenu.add_command(label="Font", command=donothing)
notemenu.add_separator()
notemenu.add_command(label="Encryption", command=donothing)
notemenu.add_command(label="Lock", command=donothing)
menubar.add_cascade(label="Notes", menu=notemenu)

# view menu
viewmenu = Menu(menubar, tearoff=False)
viewmenu.add_command(label="Minimise", command=donothing)
viewmenu.add_command(label="Full screen", command=donothing)
...
section_panel = PanedWindow(mainwindow, orient='vertical', height=800, width=150, bd=2)
...
# To add new sections
section_n = Button(section_panel, text="+ Section", command=donothing, pady=4, bg="#999999")
section_n.pack(side='bottom')
section_panel.add(section_n)
...
# note panel e0e0eb bg="#bfbfbf"
note_panel = PanedWindow(mainwindow, orient='vertical', height=800, width=150, bd=2)
...
# To add new sections
note_n = Button(note_panel, text="+ Note", command=donothing, pady=4, bg="#bfbfbf")
note_n.pack(side='bottom')
note_panel.add(note_n)
...
for i in range(0, no_section):
    new_path = "AIY/Section" + str(i + 1)
    path_folder_creator(new_path)
    print(new_path)
'''---------------------------------------------------------------------------------------------------------------------'''
...
def retrieve_data():
    text = my_text.get('1.0', END).splitlines()
    for line in text:
        print(line)


def input_data():
    text = str(input("enter data: "))
...
section_panel.add(l_section)
# To add new sections
section_n = Button(section_panel, text="print_text", command=retrieve_data, pady=4, bg="#999999")
section_n.pack(side='bottom')
section_panel.add(section_n)

section_n_2 = Button(section_panel, text="delete_text", command=delete_text, pady=4, bg="#999999")
section_n_2.pack(side='bottom')
section_panel.add(section_n_2)
...
note_panel.add(l_note)
# To add new sections
note_n = Button(note_panel, text="input_text", command=input_data, pady=4, bg="#bfbfbf")
note_n.pack(side='bottom')
note_panel.add(note_n)
...
def functions_panel_features(purpose_, list_widgets=None, direct_not=None):
    ...
    if purpose_ in purpose_title:
        ...
        if purpose_ == "+ Add":
            ...
            text_about = (
                "This is a computer project undertaken by a few students of The Foundation School, Gunjur,Bangalore. The aim of the project is to provide a note taking application that not just provide the necessary options and features that the people have already been used to but also give ways through which the user's can fully custamise their display.")
...
def text_frame_configure(present_section,present_note_section,purpose=None):
    ...
    if purpose == None:
        ...
        # activelog_note activelog_section
        print("activelog_note: ", activelog_note)
        if len(activelog_note) > 1:
            previous_note_section = activelog_note[-2]
            if len(activelog_section) > 1:
                previous_section = activelog_section[-1]
            elif len(activelog_section) == 1:
                previous_section = activelog_section[0]
        elif len(activelog_note) == 1:
            previous_note_section = None
        print("activelog_section: ",activelog_section)

        if previous_section is not None:
            if previous_note_section is not None:
                dict_note = directory_section_note_dict[previous_section]
                note_directory_previous = dict_note[previous_note_section] + "/" + previous_section + " " + previous_note_section +".txt"
        ...
        if present_section != "":
            if present_note_section != "":
                ...
                note_directory_present = dict_note[present_note_section] + "/" + present_section + " " + present_note_section + ".txt"
...
def maindisplay():
    ...
    def section_note_formation(section_or_note, text_name, present_name,section_name=None):
        ...
        if section_or_note == "section":
            ...
            [present("add", list_maindisplay_widgets, "section", present_name),text_frame_configure(present_name,'note_1'),activelog_note.append("note_1"),restart("maindisplay", list_maindisplay_widgets)])
            ...
            [present("add", list_maindisplay_widgets, "note", present_name),text_frame_configure(section_name,present_name),text_frame_configure(section_name,present_name)])
    ...
    def start_note_text_frame(count):
        if count == 0:
            activelog_section_ = "section_1"
            activelog_note_ = "note_1"
            text_frame_configure(activelog_section_,activelog_note_,'maindisplay_function_text_frame')
        else:
            pass
...
account_no_user = None
active_feature_panel = None
location_AIY_Logo = "/Users/yashlucky/Desktop/AIY LOGO_2.png"
        ...
        if purpose_ == "+ Add":
            ...
            def delete_section_note(purpose_, list_widgets):
                ...
                if len(value_combobox) > 1:
                    ...
                    Next_ = Button(feature_panel, text="Next", pady=4,
                                   command=lambda: [text_frame_configure('data_textframe_into_notefile'),
                                                    feature_panel_activity(False), delete(purpose_, list_widgets_next),
                                                    text_frame_configure('data_notefile_into_textframe')])
                    Back_ = Button(feature_panel, text="Back", pady=4,
                                   command=lambda: [feature_panel_activity(False),
                                                    restart("maindisplay", list_widgets_next)])
...
def text_frame_configure(purpose, present_section=None, present_note_section=None):
    def text_file_existance(path):
        file_accessible = None
        try:
            f = open(path, "r")
    ...
    if purpose == 'data_notefile_into_textframe':
    ...
    elif purpose == "data_textframe_into_notefile":
        if present_section is None:
            present_section = activelog_section_note_list[-1][0]
        else:
            pass
...
def maindisplay():
    ...
    note_subtract = Button(note_panel, text="- Note", pady=4,
                           command=lambda: [
                               feature_panel_activity(True, "- Remove", list_maindisplay_widgets, "direct_note")])
    note_subtract.pack(side='bottom')
    note_panel.add(note_subtract)
...
def OTP_system(username_receiver, email_id, otp_purpose):
    Username = username_receiver
    email_reciever = email_id
    sender_email = 'aiynotes2021@gmail.com'
    password = "aiynotes.gmail"
